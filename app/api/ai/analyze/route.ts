...
4. 鼓励话只能放最后，且必须简短（≤20字，一句话）
5. 不要说"你已迈出扎实第一步"、"建议加强复习"、"继续保持"等空话
6. encouragement 禁止出现不自然的口号或比喻。只能是自然的学习建议，如"先复习错词和模糊词，再学新词会更稳"

请严格按照以下 JSON 格式返回，不要有任何额外文字：
{
  "todayConclusion": "一句话直接说今天最主要的问题，例如：今天主要问题是错词和模糊词数量偏多，新词量不宜继续增加",
  ...
  "confusingWordsList": [
    {
      "word": "单词A",
      "confusingWith": "单词B",
      "reason": "一句话说明两者核心区别（只写区别，不写例句，例句由本地词库提供）"
    }
  ],
  "tomorrowPlan": {
    "newWords": 20,
...
2. confusingWordsList 只能从上面"真实易混词对"里选，不能编造。如果没有易混词数据，返回空数组 []；reason 字段只写一句核心区别，不写例句（例句由本地补充）
