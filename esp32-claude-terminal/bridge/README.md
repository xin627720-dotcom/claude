...
桥接自带一个网页聊天，手机和电脑都能用——它和 ESP32 连同一个中枢、同一个 Claude 会话，
回复**广播给所有在线端**（手机发一句，ESP32 屏幕也同步显示）。

- 本机：浏览器开 `http://localhost:8787/?token=<你的AUTH_TOKEN>`
- 手机/外网：cloudflared 暴露后开 `https://<你的隧道>/?token=<AUTH_TOKEN>`（把这网址发到手机即可）

页面从 URL 的 `?token=` 读鉴权；支持流式逐字回复，并显示 ESP32 上传的摄像头画面。

> 真·微信（公众号/企业微信）需要公网回调 + 账号注册，且微信不支持流式（整段返回）。网页方案在手机
> 浏览器即可用，通常已覆盖"手机发消息"。需要微信再单独接 webhook。
